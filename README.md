...
Данный проект поможет автоматизировать процесс разметки машин, автобусов и грузовиков в CVAT. Проект содержит скрипт `auto\_cvat\_cuboids.py`, который автоматически размечает автомобили на изображениях с помощью YOLOv8 и с не большими махинациями создает кубоиды, а далее записывает в XML-разметку в формате CVAT 1.1. А также скрипт `convert\_cvat2coco.py` который меняет формат аннотации из `CVAT for images 1.1` в `COCO`.
...
1\. Берёт все изображения из папки `IMAGE\_DIR`.
...
В начале файла находятся \_\_основные параметры\_\_:



```python

IMAGE\_DIR = r"C:\\Users\\path\\in\\image\\Train"

OUTPUT\_XML = "cvat\_perspective\_cuboids.xml"

MODEL\_NAME = "yolov8x.pt"

CONF\_THRESHOLD = 0.35

CUBOID\_ROOF\_RATIO = 0.12

```



`IMAGE\_DIR` - папка с изображениями.



`OUTPUT\_XML` - имя XML-файла, который будет создан.



`MODEL\_NAME` - модель YOLO. Сейчас используется `yolov8x.pt`.



`CONF\_THRESHOLD` - минимальная уверенность YOLO. Если значение ниже `0.35`,

объект не попадёт в разметку.



`CUBOID\_ROOF\_RATIO` - высота верхней грани кубоида внутри YOLO-бокса. Чем
...
Если нужны другие классы, их нужно добавить и в `TARGET\_CLASSES`, и в `LABELS`, а их `id` можете узнать на сайте \[YOLO](https://docs.ultralytics.com/datasets/detect/coco#usage).
...
Если `SKIP\_EDGE\_TRUNCATED = True`, скрипт пропускает машины, которые почти
...
&#x20;   <start\_frame>0</start\_frame>

&#x20;   <stop\_frame>...</stop\_frame>
...
`start\_frame` - первый кадр, сейчас всегда `0`.



`stop\_frame` - последний кадр, равен `количество изображений - 1`.
...
&#x20;   f for f in os.listdir(IMAGE\_DIR)
...
Если класс не входит в `TARGET\_CLASSES`, объект пропускается.



Если уверенность ниже `CONF\_THRESHOLD`, объект тоже пропускается.
...
build\_perspective\_cuboid(x1, y1, x2, y2, img\_w, img\_h)
...
`vp\_x` отвечает за горизонтальное положение точки схода.



`vp\_y` отвечает за вертикальное положение точки схода.
...
direction = np.array(\[vp\_x - obj\_x, vp\_y - obj\_y], dtype=np.float32)
...
roof\_y = y1 + h \* CUBOID\_ROOF\_RATIO
...
f\_tl - front top left

f\_tr - front top right

f\_bl - front bottom left

f\_br - front bottom right
...
Верхняя часть стоит на `roof\_y`.
...
np.clip(pt\[0], 0, img\_w - 1)

np.clip(pt\[1], 0, img\_h - 1)
...
`z\_order="0"` - порядок отрисовки объекта в CVAT.
...
XML: cvat\_perspective\_cuboids.xml
...
cvat\_perspective\_cuboids.xml
...
в папке `IMAGE\_DIR`. Например, если в XML указано:



```xml

<image name="frame\_0001.jpg">

```



то в задаче CVAT тоже должен быть файл `frame\_0001.jpg`.
...
This project will help automate the process of marking cars, buses and trucks in CVAT. The project contains one script `auto\_cvat\_cuboids.py `, which automatically marks cars in images using YOLOv8 and creates cuboids with little manipulation, and then writes them to XML markup in CVAT 1.1 format. And also the script `convert\_cvat2coco.py` which changes the annotation format from `CVAT for images 1.1` to `COCO`.
...
1\. Takes all images from the folder `IMAGE\_DIR'.
...
At the beginning of the file are the \_\_main parameters\_\_:
...
`IMAGE\_DIR` - folder with images.



`OUTPUT\_XML`- is the name of the XML file to be created.



`MODEL\_NAME` - is the YOLO model. Currently in use `yolov8x.pt `.



`CONF\_THRESHOLD` - minimum confidence of YOLO. If the value is lower than `0.35`,

the object will not be included in the markup.



`CUBOID\_ROOF\_RATIO` - height of the upper face of the cuboid inside the YOLO box. Than
...
If other classes are needed, they need to be added to both `TARGET\_CLASSES` and `LABELS`, and their `id` can be found on the \[YOLO](https://docs.ultralytics.com/datasets/detect/coco#usage ).
...
If `SKIP\_EDGE\_TRUNCATED = True`, the script skips cars that almost
...
`start\_frame` is the first frame, it is always `0` now.



`stop\_frame` - the last frame is equal to `number of images - 1`.
...
If the class is not included in the `TARGET\_CLASSES`, the object is skipped.



If the confidence is lower than `CONF\_THRESHOLD`, the object is also skipped.
...
`vp\_x` is responsible for the horizontal position of the vanishing point.



`vp\_y` is responsible for the vertical position of the vanishing point.
...
The upper part is set to `roof\_y`.
...
`z\_order="0"` - the order in which the object is drawn in CVAT.
...
in the `IMAGE\_DIR` folder. For example, if the XML states:
...
then there should also be a file in the CVAT task `frame\_0001.jpg `.
